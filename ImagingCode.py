...
# %% Mask pt2
'''
Removes any remaining pixels greater than 50,000 counts
'''
...
kernel = Gaussian2DKernel(sigma, x_size=7, y_size=7)
kernel.normalize()
npixels = 5
...
# %%
'''
Testing the change in bin alues
'''
...
binchange = np.array(binchange)
x = np.where(binchange == 0)
m = np.delete(Mc,x)
binchange = np.delete(binchange,x)

plt.plot(m,np.log10(binchange),'x')

# %%
'''
Plot of the number counts
'''
...
# plt.savefig('lnNvsM,largerkernel',bbox_inches='tight')
plt.show()
